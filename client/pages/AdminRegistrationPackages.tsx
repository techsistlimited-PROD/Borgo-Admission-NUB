...
export default function AdminRegistrationPackages() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (res.success && res.data) setPackages(Array.isArray(res.data) ? res.data : []);
    } catch (e) {
      console.error(e);
      toast({ title: "Error", description: "Failed to load registration packages", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-deep-plum">Registration Packages</h1>
        <div className="flex items-center gap-2">
          <Input placeholder="Search by program name" value={search} onChange={(e:any)=>setSearch(e.target.value)} />
      ...
      <Card>
        ...
        <CardContent>
          <div className="mb-4 flex gap-2">
            <select value={termFilter ?? ""} onChange={(e)=>setTermFilter(e.target.value||null)} className="border rounded p-2">
              <option value="">All Terms</option>
              {terms.map(t=> <option key={t} value={t}>{t}</option>)}
            </select>
            <select value={modeFilter ?? ""} onChange={(e)=>setModeFilter(e.target.value||null)} className="border rounded p-2">
  ...
  function updatePackageSelection(pkg: any) {
    ...
    try {
      const draft = JSON.parse(localStorage.getItem('nu_application_draft')||'{}');
      const updated = { ...draft, program: pkg.id, totalCost: pkg.totalEstimated };
      localStorage.setItem('nu_application_draft', JSON.stringify(updated));
      toast({ title: 'Package applied', description: `${pkg.program} applied to draft.` });
    } catch (e) {
      console.error(e);
      toast({ title: 'Error', description: 'Failed to apply package', variant: 'destructive' });
    }
  }
}
