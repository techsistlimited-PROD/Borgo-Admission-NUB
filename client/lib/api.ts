...
class ApiClient {
  ...
  async createApplication(data: any): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        ...
        console.warn("createApplication server failed, falling back to mock", e);
  ...
  async getPrograms(): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (res.ok) return { success: true, data: { programs: json.data || json } };
        return { success: false, error: json.error || "Failed to load programs" };
      } catch (e) {
  ...
  async getDepartments(): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (res.ok) return { success: true, data: { departments: json.data || json } };
      } catch (e) {
  ...
  async calculateCost(data: { program_code: string; department_code: string; waivers?: Array<{ type: string; value: number }> }): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        const res = await fetch("/api/programs/calculate-cost", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
        const json = await res.json().catch(() => ({}));
  ...
  async getReferrers(): Promise<ApiResponse> { return await mockApi.getReferrers(); }
  async validateReferrer(employee_id: string): Promise<ApiResponse> { return await mockApi.validateReferrer(employee_id); }
  async getReferrerStats(employee_id: string): Promise<ApiResponse> { return await mockApi.getReferrerStats(employee_id); }
  async getVisitors(params?: any): Promise<ApiResponse> { return await mockApi.getVisitors(params); }
  async createVisitor(record: any): Promise<ApiResponse> { return await mockApi.createVisitor(record); }
  async updateVisitor(id: string, updates: any): Promise<ApiResponse> { return await mockApi.updateVisitor(id, updates); }
  async deleteVisitor(id: string): Promise<ApiResponse> { return await mockApi.deleteVisitor(id); }
  async exportVisitors(params?: any): Promise<ApiResponse<any[]>> { return await mockApi.exportVisitors(params); }
  ...
  async getAdmissionSettings(): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        ...
        console.warn('getAdmissionSettings server failed, falling back to mock', e);
      }
    }
  ...
  async updateAdmissionSettings(settings: any): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        const res = await fetch('/api/admission-settings', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ settings }) });
        const json = await res.json().catch(() => ({}));
        if (res.ok) return { success: true, data: json.data };
      } catch (e) {
        console.warn('updateAdmissionSettings server failed, falling back to mock', e);
      }
    }
  ...
  async getPaymentMethods(): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (res.ok) return { success: true, data: json.data };
      } catch (e) { console.warn('getPaymentMethods server failed, falling back to mock', e); }
    }
  ...
  async createPaymentMethod(method: any): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        const res = await fetch('/api/payment-methods', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(method) });
        const json = await res.json().catch(() => ({}));
        if (res.ok) return { success: true, data: json.data };
      } catch (e) { console.warn('createPaymentMethod server failed, falling back to mock', e); }
    }
  ...
  async updatePaymentMethod(id: string, method: any): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        const res = await fetch(`/api/payment-methods/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(method) });
        const json = await res.json().catch(() => ({}));
        if (res.ok) return { success: true, data: json.data };
      } catch (e) { console.warn('updatePaymentMethod server failed, falling back to mock', e); }
    }
  ...
  async deletePaymentMethod(id: string): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (res.ok) return { success: true, data: json.data };
      } catch (e) { console.warn('deletePaymentMethod server failed, falling back to mock', e); }
    }
  ...
  async getDocumentRequirements(): Promise<ApiResponse> {
    if (this.serverAvailable) {
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (res.ok) return { success: true, data: json.data };
      } catch (e) { console.warn('getDocumentRequirements server failed, falling back to mock', e); }
    }
    return await mockApi.getDocumentRequirements();
  }

  async createDocumentRequirement(requirement: any): Promise<ApiResponse> { if (this.serverAvailable) { try { const res = await fetch('/api/document-requirements', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(requirement) }); const json = await res.json().catch(()=>({})); if (res.ok) return { success: true, data: json.data }; } catch (e) { console.warn('createDocumentRequirement server failed, falling back to mock', e); } } return await mockApi.createDocumentRequirement(requirement); }

  async updateDocumentRequirement(id: string, requirement: any): Promise<ApiResponse> { if (this.serverAvailable) { try { const res = await fetch(`/api/document-requirements/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(requirement) }); const json = await res.json().catch(()=>({})); if (res.ok) return { success: true, data: json.data }; } catch (e) { console.warn('updateDocumentRequirement server failed, falling back to mock', e); } } return await mockApi.updateDocumentRequirement(id, requirement); }
...
export const getDemoCredentials = () => ({
  applicant: { identifier: "APP123456", password: "temp123456", type: "applicant" as const },
  admin: { identifier: "admin@nu.edu.bd", password: "admin123", type: "admin" as const },
});
