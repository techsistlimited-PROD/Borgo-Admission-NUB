...
// Utility functions
export const getSyllabusByProgramId = (programId: string): Syllabus | undefined => {
...
export const calculateFirstSemesterFee = (programId: string, waiverPercentage: number = 0): {
  admissionFee: number;
  tuitionFee: number;
  labFee: number;
  otherFees: number;
  ...
  const totalCredits = firstSemesterCourses.reduce((sum, course) => sum + course.credits, 0);
  const labCourses = firstSemesterCourses.filter(course => course.type === 'lab');
...
export const createSyllabus = (syllabus: Omit<Syllabus, 'id' | 'createdAt' | 'updatedAt'>): Syllabus => {
...
export const updateSyllabus = (id: string, updates: Partial<Syllabus>): Syllabus | null => {
...
export const addCourseToSyllabus = (syllabusId: string, semesterId: string, course: Course): boolean => {
...
export const removeCourseFromSyllabus = (syllabusId: string, semesterId: string, courseId: string): boolean => {
  const syllabus = defaultSyllabuses.find(s => s.id === syllabusId);
  if (!syllabus) return false;
