...
router.post("/:provider", async (req, res) => {
  try {
    ...
    const secret =
      ...
      (process.env as any)["PAYMENT_WEBHOOK_SECRET"] ||
      null;
    ...
    if (secret) {
      ...
      console.warn(
        "No payment webhook secret configured; skipping signature validation",
      );
    }
    ...
    if (idempotencyKey) {
      const existing = await dbGet(
        `SELECT * FROM payment_webhook_events WHERE idempotency_key = ?`,
    ...
    const result = await dbRun(
      `INSERT INTO payment_webhook_events (provider, payload_json, signature_header, signature_valid, idempotency_key, received_at, status) VALUES (?, ?, ?, ?, ?, datetime('now'), ?)`,
      [
        provider,
        rawBody,
    ...
    try {
      ...
      await dbRun(
        `UPDATE payment_webhook_events SET processed_at = datetime('now'), status = 'Processed' WHERE webhook_id = ?`,
        [result.lastID],
      );
    } catch (err) {
      await dbRun(
        `UPDATE payment_webhook_events SET status = 'Failed', error = ? WHERE webhook_id = ?`,
        [String(err), result.lastID],
      );
...
router.post("/process-queue", async (req, res) => {
  try {
    ...
    const rows = await dbGet(
      `SELECT COUNT(*) as cnt FROM payment_webhook_events WHERE status = 'Queued'`,
    );
    // For simplicity, process all queued events one by one
    const queued = await (async () => {
      const db = await import("../../database/config.js");
      return db.dbAll(
        `SELECT * FROM payment_webhook_events WHERE status = 'Queued' ORDER BY received_at ASC LIMIT 100`,
        [],
      );
    })();
