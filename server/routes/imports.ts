  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const idempotencyKey =
        req.headers["idempotency-key"] || payload.idempotency_key || null;

      // Check idempotency
      if (idempotencyKey) {
        const existing = await dbGet(
          "SELECT import_job_id FROM import_jobs WHERE idempotency_key = ?",
        ...
        if (existing) {
          return res.json({
            ...
            message: "Import already processed for this idempotency key",
          });
        }
      }
      ...
      const jobResult = await dbRun(
        "INSERT INTO import_jobs (file_name, idempotency_key, status, total_rows, created_by_user_id) VALUES (?, ?, ?, ?, ?)",
        [
          fileName,
          idempotencyKey,
          "Processing",
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          failed++;
          await dbRun(
            "INSERT INTO import_job_errors (import_job_id, row_number, column_name, error_code, error_message, raw_row_json) VALUES (?, ?, ?, ?, ?, ?)",
            [
              importJobId,
              rowNumber,
              null,
      ...
      // Update job status
      await dbRun(
        "UPDATE import_jobs SET status = ?, success_rows = ?, failed_rows = ? WHERE import_job_id = ?",
        ["Completed", success, failed, importJobId],
      );
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      res.json({
        success: true,
        data: { jobs: rows, page: Number(page), limit: Number(limit), total },
      });
    } catch (error) {
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      const rows = await dbAll(
        "SELECT * FROM import_job_errors WHERE import_job_id = ? ORDER BY error_id ASC",
        [id],
      );
