  ...
  try {
    ...
    for (const table of requiredTables) {
      try {
        ...
        if (error && !error.message.includes('does not exist')) {
          ...
          throw new Error(`Required table '${table}' does not exist in Supabase database`);
        } else {
          console.log(`✅ Table '${table}' verified`);
        }
      } catch (err: any) {
    ...
    try {
      ...
      if (error) {
        ...
        console.log("ℹ️ No admission settings found - you may need to configure them in the admin panel");
      } else {
        console.log("✅ Admission settings found");
      }
    } catch (err) {
      console.log("ℹ️ Admission settings table accessible but may need configuration");
    }
    ...
    try {
      ...
      if (error) {
        ...
        console.log("ℹ️ No payment methods found - you may need to configure them in the admin panel");
      } else {
        console.log("✅ Payment methods found");
      }
    } catch (err) {
      console.log("ℹ️ Payment methods table accessible but may need configuration");
    }
    ...
    try {
      ...
      if (error) {
        console.log("⚠️ Could not verify document requirements, but table exists");
      } else if (!documents || documents.length === 0) {
        console.log("ℹ️ No document requirements found - you may need to configure them in the admin panel");
      } else {
        console.log("✅ Document requirements found");
      }
    } catch (err) {
      console.log("ℹ️ Document requirements table accessible but may need configuration");
    }
